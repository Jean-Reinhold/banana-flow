...
export const OutputDisplay: React.FC<OutputDisplayProps> = ({
  ...
  if (status === NodeStatus.COMPLETED && content) {
    if (typeof content === 'string') {
      ...
      return (
        <div className="p-4 text-sm text-gray-300 whitespace-pre-wrap overflow-y-auto w-full h-full">
          {content}
        </div>
      );
    }

    const contentObj = content as { image?: string; base64Image?: string };
    if (contentObj.image && typeof contentObj.image === 'string' && contentObj.image.startsWith('data:image')) {
      return renderMedia(contentObj.image);
    }
    if (contentObj.base64Image) {
      const dataUrl = `data:image/png;base64,${contentObj.base64Image}`;
