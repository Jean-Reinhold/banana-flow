...
/**
 * BananaFlow - Main Application Component
 * 
 * A visual node-based workflow editor for AI image and text generation.
 * Uses Google Gemini 3 Pro for AI capabilities.
 * 
 * Features:
 * - Drag-and-drop node editor
 * - Real-time workflow execution
 * - Auto-save workflow state
 * - Generation history with IndexedDB storage
 * - Customizable themes
 * 
 * @module App
 */
    ...
    case NodeType.TEXT_INPUT:
      return {
        ...
        data: {
          ...
          height: 150,
          label: 'Text Input',
          outputs: [{ id: `${id}-output`, label: 'Text', type: 'text' }],
        },
      };
    case NodeType.IMAGE_INPUT:
      return {
        ...
        data: {
          ...
          width: 350,
          label: 'Image Input',
          outputs: [{ id: `${id}-output`, label: 'Image', type: 'image' }],
        },
      };
    case NodeType.TEXT_GENERATOR:
      return {
        ...
        data: {
          ...
          inputs: [{ id: `${id}-input`, label: 'Prompt', type: 'text' }],
          outputs: [{ id: `${id}-output`, label: 'Text', type: 'text' }],
        },
      };
    case NodeType.IMAGE_EDITOR:
      return {
        ...
        data: {
          ...
          inputs: [
            { id: `${id}-input-image`, label: 'Image (Optional)', type: 'image' },
            { id: `${id}-input-text`, label: 'Prompt', type: 'text' },
          ],
          outputs: [
            { id: `${id}-output-image`, label: 'Image', type: 'image' },
            { id: `${id}-output-text`, label: 'Text', type: 'text' },
          ],
        },
      };
    case NodeType.VIDEO_GENERATOR:
      return {
        ...
        data: {
          ...
          inputs: [
            { id: `${id}-input-image`, label: 'Image', type: 'image' },
            { id: `${id}-input-text`, label: 'Text', type: 'text' },
          ],
          outputs: [{ id: `${id}-output`, label: 'Video', type: 'video' }],
        },
      };
    case NodeType.OUTPUT_DISPLAY:
      return {
        ...
        data: {
          ...
          width: 350,
          label: 'Output',
          inputs: [{ id: `${id}-input`, label: 'Input', type: 'any' }],
        },
      };
...
const App: React.FC = () => {
  ...
  // ========================================================================
  // Initialization - Load saved state from storage
  // ========================================================================
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      // Load saved workflow (nodes, edges, theme)
      const savedWorkflow = storageService.loadWorkflow();
      if (savedWorkflow) {
        setNodes(savedWorkflow.nodes || {});
        setEdges(savedWorkflow.edges || {});
        if (savedWorkflow.theme) {
          setTheme(savedWorkflow.theme);
        }
      }

      // Load generation history from IndexedDB
      const savedHistory = await storageService.loadHistory();
      if (savedHistory) {
  ...
  // ========================================================================
  // Auto-save - Persist workflow state when it changes
  // ========================================================================
  ...
  // Auto-save workflow with debounce to avoid excessive writes
  useEffect(() => {
    const timer = setTimeout(() => {
      storageService.saveWorkflow(nodes, edges, theme);
    }, AUTO_SAVE_DELAY);
  ...
  const updateNodeData = useCallback((nodeId: string, data: Partial<Node['data']>) => {
    setNodes((prev) =>
      prev[nodeId] ? { ...prev, [nodeId]: { ...prev[nodeId], data: { ...prev[nodeId].data, ...data } } } : prev
    );
  }, []);
  ...
  const onHandleMouseDown = (e: React.MouseEvent, nodeId: string, handleId: string) => {
    e.stopPropagation();
  ...
  const resolveData = async (nodeId: string, handleId: string, currentResults: Record<string, unknown>): Promise<unknown> => {
          ...
          if (allInputsReady) {
            ...
            try {
              ...
              if (node.type === NodeType.TEXT_INPUT || node.type === NodeType.IMAGE_INPUT) {
                ...
                const prompt = (await resolveData(nodeId, node.data.inputs[0].id, results)) as string;
                output = await geminiService.generateText(prompt, apiKey);
              } else if (node.type === NodeType.IMAGE_EDITOR) {
                const img = await resolveData(nodeId, node.data.inputs[0].id, results);
                const prompt = (await resolveData(nodeId, node.data.inputs[1].id, results)) as string;
                ...
                if (b64Data) {
                  const res = await geminiService.editImage(b64Data, mimeType, prompt || 'enhance this image', apiKey);
                  output = res.newBase64Image ? `data:image/png;base64,${res.newBase64Image}` : res.text;
                } else {
                  output = await geminiService.generateImage(prompt, apiKey);
                }
                ...
                output = await geminiService.generateVideo(b64, mime, prompt, (m) =>
                  updateNodeData(nodeId, { content: { progress: m } })
                );
                ...
                const res = await geminiService.executePreset(parts, node.data.prompt || '', apiKey);
                output = res.newBase64Image ? `data:image/png;base64,${res.newBase64Image}` : res.text;
              ...
              // Add to history if it's an image
              if (typeof output === 'string' && output.startsWith('data:image')) {
                const prompt = (node.data.prompt ||
                  (await resolveData(nodeId, node.data.inputs?.[0]?.id, results)) ||
                  '') as string;
                ...
                storageService.addItemToHistory(newHistoryItem);
              }
      ...
      >
        <div className="absolute top-6 right-6 z-50 flex items-center space-x-3 pointer-events-auto">
          <div className="flex items-center space-x-2 bg-neutral-900/90 backdrop-blur-xl p-2 rounded-2xl border border-white/10 shadow-2xl">
            ...
            <div className="flex items-center border-l border-white/10 ml-1 pl-2 space-x-1">
              <button
                ...
                className="p-2 text-white/60 hover:text-white transition-colors"
                title={isAppMode ? 'Canvas View' : 'App View'}
              >
          ...
          <button
            ...
            className="flex items-center px-6 py-3 font-bold text-white bg-indigo-600 hover:bg-indigo-500 rounded-2xl transition-all shadow-lg shadow-indigo-600/20 disabled:opacity-50"
          >
            {isProcessing ? (
      ...
      {!isAppMode && (
        <HistorySidebar
          ...
          onUseAsInput={(dataUrl) => {
            const newNode = createNode(NodeType.IMAGE_INPUT, { x: 100, y: 100 });
          ...
          onDeleteItem={(id) => {
            ...
            storageService.deleteItemFromHistory(id);
          }}
      ...
      {editingNodeId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-neutral-900 border border-white/10 rounded-3xl p-8 w-full max-w-2xl shadow-2xl text-white">
            ...
            <textarea
              className="w-full h-64 bg-black/40 border border-white/10 rounded-2xl p-4 text-sm focus:outline-none focus:border-indigo-500 mb-6"
              defaultValue={nodes[editingNodeId]?.data.prompt}
              onChange={(e) => {
                if (nodes[editingNodeId]) nodes[editingNodeId].data.prompt = e.target.value;
              }}
            />
            <div className="flex justify-end space-x-4">
              <button
                onClick={() => setEditingNodeId(null)}
                className="px-6 py-2 rounded-xl bg-white/5 hover:bg-white/10"
              >
                Cancel
              </button>
              <button
                onClick={() => saveEditedPrompt(nodes[editingNodeId!]?.data.prompt || '')}
