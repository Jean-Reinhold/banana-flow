...
import { TextIcon, ImageIcon, MagicIcon, VideoIcon, OutputIcon, MuteIcon } from './icons';
    ...
    >
      ...
      <span
        className={`absolute top-1/2 -translate-y-1/2 ${
          isInput ? 'left-full ml-4' : 'right-full mr-4'
        } text-xs text-white bg-black/50 px-2 py-1 rounded-md whitespace-nowrap opacity-0 group-hover/handle:opacity-100 transition-opacity pointer-events-none`}
      >
        {label}
      </span>
    </div>
...
const NodeComponent: React.FC<NodeProps> = ({
  ...
  const config = useMemo(() => {
    switch (node.type) {
      case NodeType.TEXT_INPUT:
        return { icon: <TextIcon className="w-4 h-4" />, title: data.label };
      case NodeType.IMAGE_INPUT:
        return { icon: <ImageIcon className="w-4 h-4" />, title: data.label };
      case NodeType.TEXT_GENERATOR:
        return { icon: <MagicIcon className="w-4 h-4" />, title: data.label };
      case NodeType.IMAGE_EDITOR:
        return { icon: <MagicIcon className="w-4 h-4" />, title: data.label };
      case NodeType.PROMPT_PRESET:
        return { icon: null, title: data.label };
      case NodeType.VIDEO_GENERATOR:
        return { icon: <VideoIcon className="w-4 h-4" />, title: data.label };
      case NodeType.OUTPUT_DISPLAY:
        return { icon: <OutputIcon className="w-4 h-4" />, title: data.label };
      default:
        return { icon: null, title: 'Unknown' };
    }
  ...
  useEffect(() => {
    ...
    } else if (data.content && typeof data.content === 'object' && 'image' in data.content) {
      setImageUrl((data.content as { image: string }).image);
    } else {
      setImageUrl(null);
    }
    return () => {
  ...
  const statusClasses = useMemo(() => {
    switch (data.status) {
      case NodeStatus.PROCESSING:
        return 'border-indigo-500/80 ring-2 ring-indigo-500/50 animate-pulse';
      ...
      case NodeType.OUTPUT_DISPLAY:
        return (
          <OutputDisplay
            ...
            progressMessage={
              data.content && typeof data.content === 'object' && 'progress' in data.content
                ? (data.content as { progress: string }).progress
                : undefined
            }
          />
        );
  ...
  const height =
    data.height || (node.type === NodeType.IMAGE_INPUT || node.type === NodeType.OUTPUT_DISPLAY ? 350 : 150);
  const isPresetNode = node.type === NodeType.PROMPT_PRESET;

  return (
    <>
      <div
        className={`absolute flex flex-col group/node-container ${isPresetNode ? 'items-center' : ''}`}
        style={{
      ...
      >
        <div
          className="flex items-center gap-2 mb-2 px-1 cursor-move relative w-full"
          style={{ color: 'var(--node-text-color, #ffffff)' }}
          onMouseDown={(e) => onMouseDown(e, node.id)}
        >
          {config.icon}
          <h3 className="text-xs font-bold truncate flex-grow">{config.title}</h3>
          ...
          <button
            ...
            className="opacity-0 group-hover/node-container:opacity-100 p-1 hover:bg-red-500/20 rounded transition-all text-white/50 hover:text-red-500"
            title="Delete Node"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div
          ...
          className={`relative group/node backdrop-blur-xl shadow-[0_0_30px_rgba(0,0,0,0.5)] transition-all duration-200 ease-in-out border-2 ${statusClasses} ${
            data.isMuted ? 'opacity-50' : ''
          } ${isPresetNode ? 'rounded-full' : 'rounded-2xl'}`}
          style={{
            width: `${width}px`,
            height: `${height}px`,
