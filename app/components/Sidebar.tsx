...
const SidebarButton: React.FC<{
  ...
  >
    {icon}
    <span className="ml-3 text-sm truncate" title={label}>
      {label}
    </span>
  </button>
);
...
const Sidebar: React.FC<SidebarProps> = ({ onAddNode, isCollapsed, onToggle }) => {
  return (
    <div
      className={`relative z-20 flex flex-col flex-shrink-0 h-full bg-black/30 backdrop-blur-2xl border-r border-white/10 transition-all duration-300 ease-in-out ${
        isCollapsed ? 'w-0' : 'w-60 p-4'
      }`}
    >
      <div
