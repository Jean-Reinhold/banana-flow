...
const ResizingNodeContainer: React.FC<{
  node: Node;
  children: React.ReactNode;
  nodeRgba: string;
}> = ({ node, children, nodeRgba }) => {
  const { data } = node;
  ...
  const imageUrl = useMemo(() => {
    ...
    if (node.type === NodeType.IMAGE_INPUT) {
      ...
      if (typeof data.content === 'string' && data.content.startsWith('data:image')) url = data.content;
        ...
        url = (data.content as { image: string }).image;
    }
    return url;
  ...
  const heightStyle =
    isImageNode && dynamicHeight ? { height: `${dynamicHeight}px` } : { height: `${data.height || defaultHeight}px` };
  ...
  return (
    ...
    >
      <div className="relative w-full h-full overflow-hidden rounded-2xl">{children}</div>
    </div>
  );
};
...
const AppModeView: React.FC<AppModeViewProps> = ({ nodes, updateNodeData, theme }) => {
  ...
  const getNodeIcon = (nodeType: NodeType) => {
    switch (nodeType) {
      ...
      case NodeType.OUTPUT_DISPLAY:
        return <OutputIcon className="w-4 h-4" />;
      default:
        return null;
    }
  };
